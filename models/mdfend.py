...
class MultiDomainFENDModel(torch.nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        for i in range(self.num_expert):
            ...
            shared_feature += (tmp_feature * gate_value[:, i].unsqueeze(1))  #gate_value会沿着第二个维度进行广播，变成 [bs,num_expert* ] 的形状
...
class Trainer():
    ...
    def train(self, logger = None):
        ...
        for epoch in range(self.epoches):
            ...
            train_test_val_result.append({'epoch_idx':epoch,'loss':avg_loss.item(),'details':results})
            mark = recorder.add(results)

            if mark == 'save':
                torch.save(self.model.state_dict(),os.path.join(self.save_param_dir, 'parameter_mdfend_me.pkl'))
            elif mark == 'esc':
                # break #weibo21 根据early_stop_steps早停
                pass

            if abs(loss_list[-1] - loss_list[-2]) < 0.001:  # 课堂新数据集根据loss早停
                break
            else:
                continue


        json_str = json.dumps(train_test_val_result, ensure_ascii=False)
        with open('train_loss_and_val.json', 'w', encoding='utf-8',) as f:
            f.write(json_str)
        f.close()
        # 加载模型参数
        self.model.load_state_dict(torch.load(os.path.join(self.save_param_dir, 'parameter_mdfend_me.pkl')))
        print('---------------------------test_result---------------------')
        results = self.test(self.test_loader)
        print(results)
        test_json_str = json.dumps(results, ensure_ascii=False)
        with open('test.json', 'w', encoding='utf-8') as f:
            f.write(test_json_str)
        f.close()
        return results, os.path.join(self.save_param_dir, 'parameter_mdfend_me.pkl')
